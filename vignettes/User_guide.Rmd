...
`experiment_table.RDS` represents one simulation case. For one case, the main
flow is:
...
3. Each row contains a resolved `case_spec` and a generated `community_object`.
4. `create_environments_from_spec()` creates environmental temperature series,
   keyed by `env_series_id`.
5. `simulate_dynamics_from_spec()` loops over the experiment-table rows. For
