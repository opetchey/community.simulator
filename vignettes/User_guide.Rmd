...
where \(N_i(t)\) is the abundance of species \(i\), \(\alpha_{ij}\) is the LV
interaction matrix, \(I\) is `simulation.immigration_rate`, and
\(\epsilon = 10^{-6}\), \(\beta = 0.001\), and \(\delta = 0.001\) are fixed
internal constants.
...
For continuous immigration, \(I\) is `simulation.immigration_rate` per unit
time. For pulse immigration, the same value is added as a pulse at output times.
Temperature \(T_t\) is obtained from the generated temperature series using the
selected `simulation.temperature_interpolation` method.
...
\(\rho_j\) is resource renewal rate, \(K_{R,j}\) is resource supply, and \(I_N\)
is `simulation.consumer_immigration_rate`.
