...
| Run a simulation interactively in a ShinyApp | [`run_simulation_explorer()`](https://opetchey.github.io/community.simulator/reference/run_simulation_explorer.md) |
...
- $`b_i(T)`$: birth rate of species $`i`$ at temperature $`T`$.
- $`d_i(T)`$: death rate of species $`i`$ at temperature $`T`$.
- $`r_i(T)`$: intrinsic growth rate of species $`i`$ at temperature
  $`T`$, before the small numerical offset used inside the LV
  simulators.
- $`a_{b,i}`$: maximum birth rate of species $`i`$. This is the
  species-level value drawn from the distribution specified by
  `birth_maximum_mean`, `birth_maximum_range`, and
  `birth_maximum_distribution`.
- $`b_{\mathrm{opt},i}`$: thermal optimum of the birth-rate curve for
  species $`i`$. This is drawn from the distribution specified by
  `birth_optimum_mean`, `birth_optimum_range`, and
  `birth_optimum_distribution`.
- $`sd_{\mathrm{perf},i}`$: width of the Gaussian birth-rate curve for
  species $`i`$. This is drawn from the distribution specified by
  `birth_width_mean`, `birth_width_range`, and
  `birth_width_distribution`.
- $`a_{d,i}`$: death-rate intercept for species $`i`$. In current YAML
  specifications this comes from `death_intercept` and is shared across
  species.
- $`z_i`$: temperature sensitivity of the death-rate curve for species
  $`i`$. In current YAML specifications this comes from
  `death_temperature_slope` and is shared across species.
...
- $`i`$: focal species.
- $`j`$: species whose abundance affects species $`i`$.
- $`t`$: discrete time step.
- $`T_t`$: environmental temperature at time step $`t`$.
- $`N_i(t)`$: abundance of species $`i`$ at time step $`t`$.
- $`N_j(t)`$: abundance of species $`j`$ at time step $`t`$.
- $`b_i(T_t)`$: temperature-dependent birth rate of species $`i`$.
- $`d_i(T_t)`$: temperature-dependent death rate of species $`i`$.
- $`\tilde{r}_i(T_t)`$: net intrinsic growth rate of species $`i`$,
  after adding the small fixed offset $`\epsilon`$.
- $`K_i(T_t)`$: temperature-dependent carrying-capacity scaling term for
  species $`i`$.
- $`\alpha_{ij}`$: LV interaction coefficient describing the effect of
  species $`j`$ on species $`i`$.
- $`g_i(t)`$: per-step exponential growth term for species $`i`$.
- $`I`$: immigration added to species $`i`$ each time step, specified as
  `simulation.immigration_rate`.
- $`\epsilon`$: fixed net-growth offset, currently $`10^{-6}`$.
- $`\beta`$: fixed density-dependence scaling constant, currently
  $`0.001`$.
- $`\delta`$: fixed density-dependence scaling constant, currently
  $`0.001`$.
...
- `ode_maxsteps`: maximum number of internal solver steps before
  reaching the next output time. Increase this for dense or fast
  environmental forcing when `lsoda` reports excessive work or `MXSTEP`.
...
- $`\alpha_{ij}`$: LV interaction coefficient describing the effect of
  species $`j`$ on species $`i`$.
- $`I`$: immigration term. For continuous immigration this is
  `simulation.immigration_rate` per unit time; for pulse immigration the
  same value is added as a pulse at output times.
- $`\epsilon`$: fixed net-growth offset, currently $`10^{-6}`$.
- $`\beta`$: fixed density-dependence scaling constant, currently
  $`0.001`$.
- $`\delta`$: fixed density-dependence scaling constant, currently
...
- $`i`$: focal consumer species.
- $`j`$: focal resource.
- $`t`$: continuous simulation time.
- $`T`$: temperature used to describe a generic uptake curve.
- $`T_t`$: environmental temperature at time $`t`$.
- $`N_i(t)`$: abundance of consumer $`i`$ at time $`t`$.
- $`R_j(t)`$: concentration of resource $`j`$ at time $`t`$.
- $`dN_i/dt`$: instantaneous rate of change in abundance for consumer
  $`i`$.
- $`dR_j/dt`$: instantaneous rate of change in concentration for
  resource $`j`$.
- $`u_i(T)`$: simplified temperature-dependent uptake curve for consumer
  $`i`$, shown to introduce the uptake-curve form.
- $`u_{\max,i}`$: maximum uptake parameter for the simplified uptake
  curve of consumer $`i`$.
- $`u_{\mathrm{opt},i}`$: thermal optimum for the simplified uptake
  curve of consumer $`i`$.
- $`sd_{u,i}`$: thermal width of the simplified uptake curve of consumer
  $`i`$.
- $`u_{\max,ij}(T_t)`$: temperature-dependent maximum uptake of resource
  $`j`$ by consumer $`i`$ at time $`t`$.
- $`a_{u,ij}`$: height, or maximum value, of the uptake curve for
  consumer $`i`$ on resource $`j`$.
- $`u_{\mathrm{opt},ij}`$: thermal optimum of the uptake curve for
  consumer $`i`$ on resource $`j`$.
- $`sd_{u,ij}`$: thermal width of the uptake curve for consumer $`i`$ on
  resource $`j`$.
- $`U_{ij}(T_t, R_j)`$: realised per-capita uptake rate of resource
  $`j`$ by consumer $`i`$, after accounting for temperature and current
  resource concentration.
- $`q_{ij}`$: resource-use matrix entry describing how strongly consumer
  $`i`$ uses resource $`j`$.
- $`h_{ij}`$: Monod half-saturation constant for consumer $`i`$ on
  resource $`j`$.
- $`e_i`$: conversion efficiency for consumer $`i`$.
- $`d_i`$: consumer death rate for consumer $`i`$.
- $`I_N`$: consumer immigration term, specified as
  `simulation.consumer_immigration_rate`.
- $`\rho_j`$: renewal rate of resource $`j`$.
- $`K_{R,j}`$: supply concentration of resource $`j`$.
...
The consumer-resource model supports three resource-use modes. These
modes are different ways of constructing the resource-use matrix
$`q_{ij}`$ that appears in the CR dynamic model. In the uptake equation
above, $`q_{ij}`$ multiplies the temperature- and resource-dependent
uptake rate, so it controls how strongly consumer $`i`$ uses resource
$`j`$.

- `"one_resource_all_consumers"`: all consumers use one active resource.
  The active resource has $`q_{ij} = 1`$ for every consumer $`i`$, and
  inactive resources have $`q_{ij} = 0`$.
- `"diagonal"`: each consumer uses its corresponding resource. The
  diagonal entries have $`q_{ii} = 1`$, and off-diagonal entries have
  $`q_{ij} = 0`$.
- `"shared_to_private"`: each consumer splits uptake between one shared
  resource and a species-specific private resource. If $`p_i`$ is the
  private resource-use fraction for consumer $`i`$, then the
  shared-resource entry has $`q_{ij} = 1 - p_i`$, the private-resource
  entry has $`q_{ij} = p_i`$, and all other entries have $`q_{ij} = 0`$.
...
current experiment workflow uses a memory-efficient spectral `1/f`
process. The raw series is rescaled after generation, so the realized
mean and sample standard deviation of each generated temperature series
match the requested values. The main temperature fields are:
...
each row of `experiment_table.RDS` represents one simulation case. The
workflow also writes lean runtime tables so environment creation and
simulation do not need to keep the full table, including resolved
`case_spec` objects, in memory. For one case, the main flow is:
...
YAML experiment specification
  ...
  -> create_experiment_table_from_spec()
       ...
       -> community_object
       -> environment_table.RDS
       -> simulation_table.RDS
...
one simulation-table row + community_object + matching temperature series
...
3.  The full experiment table contains a resolved `case_spec` and
    generated `community_object`; lean runtime tables separate unique
    environment series, simulation rows, and unique community objects.
4.  [`create_environments_from_spec()`](https://opetchey.github.io/community.simulator/reference/create_environments_from_spec.md)
    creates environmental temperature series from
    `environment_table.RDS`, keyed by `env_series_id`.
5.  [`simulate_dynamics_from_spec()`](https://opetchey.github.io/community.simulator/reference/simulate_dynamics_from_spec.md)
    loops over the simulation-table rows. For each case it selects the
...
If you are working from the RStudio project in a local checkout, you can
also source the project launcher:
...
community** to create one community and inspect its structure, traits,
thermal performance curves, and community performance curve. Press
**Simulate** only when you want to run the dynamics. If a simulation is
running for too long, press **Stop simulation** to interrupt the
background process.
...
- **Detailed**: model-specific controls, including LV interaction type,
  ...
  private-resource use distribution, half-saturation, resource supply,
  and related parameters.

For the consumer-resource model, the community performance curve is
shown as a binary viability summary: at each temperature it gives the
fraction of consumers with positive expected growth.
...
| `simulation.ode.maxsteps` | Continuous LV, CR | Optional maximum number of internal solver steps before reaching the next output time. Increase this if `lsoda` reports excessive work or `MXSTEP`. |
...
- `environment_table.RDS`: one row per unique environmental time series
  to generate.
- `simulation_table.RDS`: one row per simulation case, without the large
  resolved specification and community-object list columns.
- `community_objects.RDS`: one row per unique community object, keyed by
  `community_id`.
...
Only available for use in macOS and Linux. Windows users can still run
experiments, but parallel processing is not supported.
