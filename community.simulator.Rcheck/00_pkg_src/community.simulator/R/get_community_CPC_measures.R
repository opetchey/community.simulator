...
get_community_CPC_measures <- function(temperatures,
  ...
  for (i in seq_along(expt$case_id)) {
    ...
    species_pars2 <- suppressMessages(
    ...
    species_pars4 <- tot_performance |>
      dplyr::group_by(case_id) |>
      dplyr::summarise(CV_community_perf_info = stats::sd(community_igr_info) / mean(community_igr_info),
                       CV_community_perf_naive = stats::sd(community_igr_naive) / mean(community_igr_naive),
                       .groups = "drop")
